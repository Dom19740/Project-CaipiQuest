...
import RoomManager from '@/components/RoomManager'; // Import RoomManager
...
const CaipiQuest: React.FC = () => {
  ...
  const handleRoomJoined = (id: string, name: string) => {
    setRoomId(id);
    setPlayerName(name);
    showSuccess(`Welcome, ${name}! You are in room: ${id}`);
  };
        ...
        <div className="flex flex-col lg:flex-row gap-8 items-start">
          ...
          <Card className="w-full lg:w-80 bg-white/80 backdrop-blur-sm shadow-lg border-lime-300 border-2">
            ...
            <CardContent className="p-4">
              <p className="text-gray-600 mb-2">Room: <span className="font-semibold text-lime-700">{roomId}</span></p>
              <p className="text-gray-600 mb-4">Player: <span className="font-semibold text-lime-700">{playerName}</span></p>
